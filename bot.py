...
TOKEN = "TU_TOKEN_AQUI"
LOG_CHAT_ID = -1001234567890  # Cambia por tu chat de logs
...
async def warn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    warnings = user_warnings[key]await update.message.reply_text(f"⚠️ Advertencia {warnings}/{MAX_WARNINGS}")
...
async def filter_bad_words(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(word in text for word in bad_words):
        ...
        await context.bot.send_message(LOG_CHAT_ID, f"Mensaje eliminado por palabra prohibida:update.message.text")async def anti_flood(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def track_bot(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status == ChatMember.MEMBER:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="¡Gracias por agregarme!")if _name_ == "_main_":
